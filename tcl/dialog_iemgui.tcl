...
proc ::dialog_iemgui::pdtk_iemgui_dialog {mytoplevel mainheader dim_header_UNUSED \
    ...
    # handle color values - use default flag from backend, and palette color for display if default
    set ::dialog_iemgui::var_color_background($vid) [expr {$bcol_default ? $::pd_colors::palette(background) : $bcol}]
    set ::dialog_iemgui::var_color_background_default($vid) $bcol_default

    set ::dialog_iemgui::var_color_foreground($vid) [expr {$fcol_default ? $::pd_colors::palette(foreground) : $fcol}]
    set ::dialog_iemgui::var_color_foreground_default($vid) $fcol_default

    set ::dialog_iemgui::var_color_label($vid) [expr {$lcol_default ? $::pd_colors::palette(foreground) : $lcol}]
