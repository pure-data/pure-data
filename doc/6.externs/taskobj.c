...
static void taskobj_progress_workfn(t_task *task, float *data)
{
    ...
    while (elapsed < ms && task_check(task))
    {
    #ifdef _WIN32
        Sleep(delta);
    #else
        usleep(delta * 1000.0);
    #endif
            /* normally we would pass a heap allocated object to task_notify(),
            but if the data is just a single integer, we can pass it as a t_int. */
        progress = elapsed / ms * 100;
        task_notify(task, (void*)progress, (t_task_callback)taskobj_progress_notify);
        elapsed += delta;
    }
}

static void taskobj_progress_callback(t_taskobj *x, float *data)
{
    if (x) /* completed */
    {
        x->x_task = 0;
...
static void taskobj_progress(t_taskobj *x, t_floatarg ms)
{
    ...
    data = (float *)getbytes(sizeof(*data));
    *data = ms;
        /* spawn task and store task handle */
    x->x_task = task_spawn((t_pd *)x, data, (t_task_workfn)taskobj_progress_workfn,
