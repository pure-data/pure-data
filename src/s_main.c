...
static int sys_tasksystem_threads = DEFNUMWORKERTHREADS;
...
# endif /* _MSC_VER */
   ...
   if (sys_hipriority)
    ...
    if (sys_tasksystem_threads > 0 && !sys_batch)
        sys_tasksystem_start(sys_tasksystem_threads); /* start task queue */
...
int sys_argparse(int argc, const char **argv)
{
    ...
    while ((argc > 0) && **argv == '-')
    {
        ...
        else if (!strcmp(*argv, "-nobatch"))
        {
            ...
            sscanf(argv[1], "%d", &sys_tasksystem_threads) >= 1)
        {
            if (sys_tasksystem_threads < 0)
            {
                fprintf(stderr, "warning: bad argument '%d' for '-workerthreads'; using default.",
                    sys_tasksystem_threads);
                sys_tasksystem_threads = DEFNUMWORKERTHREADS;
            }
            argc -= 2;
            argv += 2;
        }
