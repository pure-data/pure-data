...
typedef struct _knb
{
    ...
    unsigned int      x_move_mode:2; /* 0=X 1=Y 2=X+Y 3=angle */
...
static void knb_draw_io(t_knb *x,t_glist *glist, int old_snd_rcv_flags)
{
    ...
    {
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -width %d -tags [list %lxOUTLINE %lxOBJ %lxSELECT]\n",
            canvas, xpos, ypos,
    ...
    {
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -fill black -tags [list %lxOUT%d %lxOBJ]\n",
            canvas,
    ...
    {
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -fill black -tags [list %lxIN%d %lxOBJ]\n",
            canvas,
            xpos, ypos,
...
static void knb_update_ticks(t_knb *x, t_glist *glist)
{
    ...
    for (tick = 0; tick < x->x_ticks; tick++)
    {
        ...
        xTc = xc + r1 * cos(alpha);
        yTc = yc + r1 * sin(alpha);
        sys_vgui(".x%lx.c create oval %d %d %d %d -fill #%06x -outline #%06x -tags [list %lxTICKS %lxOBJ]\n",
            canvas,
            NEAR(xTc - r2), NEAR(yTc - r2),
            NEAR(xTc + r2), NEAR(yTc + r2),
            x->x_gui.x_fcol, x->x_gui.x_fcol, x, x);
    }
...
{
    t_canvas *canvas = glist_getcanvas(glist);
    sys_vgui(".x%lx.c create oval 0 0 0 0 -tags [list %lxBASE %lxOBJ %lxSELECT]\n",
        canvas, x, x, x);
    knb_draw_io(x, glist, 0);
    sys_vgui(".x%lx.c create arc 0 0 0 0 -tags [list %lxARC %lxOBJ]\n",
        canvas, x, x);
    sys_vgui(".x%lx.c create oval 0 0 0 0 -tags [list %lxCENTER %lxOBJ]\n",
        canvas, x, x);
    sys_vgui(".x%lx.c create line 0 0 0 0 -tags [list %lxWIPER %lxOBJ]\n",
        canvas, x, x);
...
static void knb_draw_select(t_knb *x,t_glist *glist)
{
    ...
    sys_vgui(".x%lx.c itemconfigure %lxSELECT -outline #%06x\n", canvas, x, col);
...
static void knb_properties(t_gobj *z, t_glist *owner)
{
    ...
    sprintf(buf, "pdtk_iemgui_dialog %%s knb {}" /* x objname (dimension_label) */
            ...
            0, /*no_schedule*/
            x->x_lin0_log1,
            x->x_gui.x_isa.x_loadinit, x->x_move_mode,
            -1,
...
static void knb_dialog(t_knb *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    int movemode = (int)atom_getintarg(17, argc, argv);
    ...
    if (movemode < 0) x->x_move_mode = 0;
    else if (movemode > 3) x->x_move_mode = 3;
    else x->x_move_mode = movemode;
...
static void knb_motion(t_knb *x, t_floatarg dx, t_floatarg dy)
{
    ...
    switch(x->x_move_mode) {
        case 0: d = dx; break;
        case 1 : break;
        case 2 : if (fabs(dx) > fabs(dy)) d = dx; break;
    }
...
static void knb_move_mode(t_knb *x, t_floatarg movemode)
{
    float fval = knb_getfval(x);
    if (movemode < 0) x->x_move_mode = 0;
    else if (movemode > 3) x->x_move_mode = 3;
...
static void *knb_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    float v = 0;
    int movemode = 2, ticks = 0, arcwidth = 0, start_angle = -135, end_angle = 135;
...
void g_knb_setup(void)
{
    ...
    class_addmethod(knb_class, (t_method)knb_move_mode, gensym("move_mode"), A_FLOAT, 0);
