...
static void vinlet_bang(t_vinlet *x) { outlet_bang(x->x_obj.ob_outlet); }
...
static void vinlet_free(t_vinlet *x)
{
    ...
    if(x->x_buf) t_freebytes(x->x_buf, x->x_bufsize * sizeof(*x->x_buf));
...
/* ------------------------- signal inlet -------------------------- */
int vinlet_issignal(t_vinlet *x) { return (x->x_buf != 0); }
...
static void vinlet_setup(void)
{
    ...
    class_addcreator(
        (t_newmethod) vinlet_newsig, gensym("inlet~"), A_DEFSYM, 0);
    ...
    class_addmethod(
        vinlet_class, (t_method) vinlet_fwd, gensym("fwd"), A_GIMME, 0);
    class_addmethod(
        vinlet_class, (t_method) vinlet_dsp, gensym("dsp"), A_CANT, 0);
...
static void voutlet_bang(t_voutlet *x) { outlet_bang(x->x_parentoutlet); }
...
int voutlet_issignal(t_voutlet *x) { return (x->x_buf != 0); }
        ...
        if(parentsigs)
        {
            ...
            else
            {
                int method = (x->x_updown.method == 3
                                  ? (pd_compatibilitylevel < 44 ? 0 : 1)
                                  : x->x_updown.method);
                dsp_add(voutlet_doepilog_resampling, 2, x,
                    (t_int) re_parentvecsize);
...
static void *voutlet_newsig(t_symbol *s)
{
    ...
    x->x_canvas = canvas_getcurrent();
    x->x_parentoutlet =
        canvas_addoutlet(x->x_canvas, &x->x_obj.ob_pd, &s_signal);
...
static void voutlet_setup(void)
{
    ...
    class_addcreator(
        (t_newmethod) voutlet_newsig, gensym("outlet~"), A_DEFSYM, 0);
    ...
    class_addmethod(
        voutlet_class, (t_method) voutlet_dsp, gensym("dsp"), A_CANT, 0);
