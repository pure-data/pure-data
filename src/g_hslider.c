...
static void hslider_draw_displace(t_hslider *x, t_glist *glist, int dx, int dy)
{
    t_canvas *canvas = glist_getcanvas(glist);
...
void hslider_draw(t_hslider *x, t_glist *glist, int mode, int dx, int dy)
{
    if(mode == IEM_GUI_DRAW_MODE_DISPLACE)
        hslider_draw_displace(x, glist, dx, dy);
