    ...
    if ((y = canvas_findhitbox(x, xpos, ypos, &x1, &y1, &x2, &y2)))
    {
        ...
        else
        {
                        ...
                        xpos >= x2-4 && ypos < y2-4)
            {
                if (doit)
                {
                    t_selection *sel = NULL;
                    t_selection *saved = NULL;
                    int x1, y1, x2, y2;
                    canvas_undo_add(x, UNDO_SEQUENCE_START, "resize", 0);
                    if (!glist_isselected(x, y))
                    {
                        glist_select(x, y);
                    }
                    ...
                    while (x->gl_editor->e_selection)
                    {
                        t_selection *newsel = (t_selection *)getbytes(sizeof(*sel));
                        if(newsel)
                        {
                            newsel->sel_next = saved;
                            newsel->sel_what = x->gl_editor->e_selection->sel_what;
                            saved = newsel;
                        }
                        glist_deselect(x, x->gl_editor->e_selection->sel_what);
                    }
                    
                    for (sel = saved; sel; sel = sel->sel_next)
                    {
                        glist_select(x, sel->sel_what);
                        canvas_undo_add(x, UNDO_APPLY, "resize",
                            canvas_undo_set_apply(x, glist_getindex(x, sel->sel_what)));
                        glist_deselect(x, sel->sel_what);
                    }
                    
                    while (saved)
                    {
                        t_selection *next = saved->sel_next;
                        glist_select(x, saved->sel_what);
                        freebytes(saved, sizeof(*saved));
                        saved = next;
                    }
                    ...
                    canvas_undo_add(x, UNDO_SEQUENCE_END, "resize", 0);
                }
