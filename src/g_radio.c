...
static void radio_draw_new(t_radio *x, t_glist *glist)
{
    ...
    for(i = 0; i < n; i++)
    {
        ...
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -width %d -fill #%06x -tags [list %lxBASE%d %lxOBJ]\n",
            canvas, xx11, yy11, xx12, yy12, IEMGUI_ZOOM(x),
            x->x_gui.x_bcol, x, i, x);
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -fill #%06x -outline #%06x -tags [list %lxBUT%d %lxOBJ]\n",
    ...
    if(!x->x_gui.x_fsf.x_snd_able)
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -fill black -tags [list %lxOUT%d %lxOBJ outlet]\n",
             canvas,
             xx11b, yy11 + IEMGUI_ZOOM(x) - ioh,
             xx11b + iow, yy11,
            x, 0, x);
    if(!x->x_gui.x_fsf.x_rcv_able)
        sys_vgui(".x%lx.c create rectangle %d %d %d %d -fill black -tags [list %lxIN%d %lxOBJ inlet]\n",
             canvas,
             xx11b, yy11b,
             xx11b + iow, yy11b - IEMGUI_ZOOM(x) + ioh,
            x, 0, x);

    sys_vgui(".x%lx.c create text %d %d -text {%s} -anchor w -font {{%s} -%d %s} -fill #%06x -tags [list %lxLABEL %lxOBJ label text]\n",
        canvas,
