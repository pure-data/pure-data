    ...
    if (INTER->i_nfdpoll) {
        ...
        for (i = 0; i < INTER->i_nfdpoll && !INTER->i_fdschanged; i++)
            if (FD_ISSET(INTER->i_fdpoll[i].fdp_fd, &readset)) {
                (*INTER->i_fdpoll[i].fdp_fn)(INTER->i_fdpoll[i].fdp_ptr,
...
#        endif
    ...
    if (sched_setscheduler(0,
            (mode == MODE_NRT ? SCHED_OTHER : SCHED_FIFO), &par)
        < 0) {
...
void sys_sockerror(char const* s)
{
        ...
        if (ret < 0) {
            ...
            if (buf[ret - 1] != '\n') {
                ...
                if (semi)
                    *semi = 0;
                if (x->sr_fromaddrfn)
                    (*x->sr_fromaddrfn)(x->sr_owner, (void const*)x->sr_fromaddr);
...
int sys_startgui(char const* libdir)
{
    ...
    if (sys_do_startgui(libdir))
        return (-1);
    for (x = pd_getcanvaslist(); x; x = x->gl_next)
        if (strcmp(x->gl_name->s_name, "_float_template") && strcmp(x->gl_name->s_name, "_float_array_template") && strcmp(x->gl_name->s_name, "_text_template")) {
            glist_maybevis(x);
            canvas_vis(x, 1);
        }
    return (0);
