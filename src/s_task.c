...
int sys_taskqueue_perform(t_pdinstance *pd, int nonblocking)
{
    ...
    while (queue->tq_running)
    {
        ...
        do_task:
            ...
            if (!atomic_int_load(&task->t_cancel))
            {
                task->t_workfn(task, task->t_data);
                didsomething = 1;
            }

            if (atomic_int_load(&task->t_suspend) > 0) /* suspended */
            {
                    /* NB: it is unlikely, but entirely possible, that the task
                    is already fully resumed at this point, see task_resume() */
                if (atomic_int_fetch_sub(&task->t_suspend, 1) - 1 == 0) /* fully resumed */
                {
                    if (task->t_workfn && !atomic_int_load(&task->t_cancel)) /* continue */
                        goto do_task;
                    else
...
static void taskqueue_dopoll(t_taskqueue *queue)
{
    ...
    while (msg)
    {
        ...
        int canceled = atomic_int_load(&msg->m_task->t_cancel);
        msg->m_fn(canceled ? 0 : msg->m_task->t_owner, msg->m_data);
    ...
    while (task)
    {
        ...
        if (task->t_cb)
        {
            int canceled = atomic_int_load(&task->t_cancel);
            task->t_cb(canceled ? 0 : task->t_owner, task->t_data);
        }
...
void task_suspend(t_task *task)
{
    ...
    while (atomic_int_load(&task->t_suspend) == 0)
    {
  #ifdef _WIN32
        Sleep(POLL_INTERVAL);
  #else
...
void task_resume(t_task *task, t_task_workfn workfn)
{
    ...
    if (queue->tq_running)
    {
        ...
        if (atomic_int_fetch_sub(&task->t_suspend, 1) - 1 == 0) /* fully resumed */
        {
            if (workfn && !atomic_int_load(&task->t_cancel)) /* continue */
            {
                t_tasklist *fromsched = &queue->tq_fromsched;
...
#endif /* PD_WORKERTHREADS */
    atomic_int_store(&task->t_suspend, 1);
}
