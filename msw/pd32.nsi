...
OutFile "/tmp/pd-${PRODUCT_VERSION}.windows-installer.exe"
InstallDir "$PROGRAMFILES\Pd-32bit"
;
; removed as of 0.49 to prevent 32/64 installer clashing. No negative side effects were observed.
;
;
...
SectionGroup /e "${COMPONENT_GROUP_TEXT}"
  ...
  Section "${COMPONENT_STARTMENU_TEXT}" StartMenu
    ...
    CreateDirectory "$SMPROGRAMS\Pd-32bit"
    CreateShortCut "$SMPROGRAMS\Pd-32bit\Website.lnk" "$INSTDIR\${PRODUCT_NAME}.url"
    CreateShortCut "$SMPROGRAMS\Pd-32bit\Uninstall.lnk" "$INSTDIR\uninst.exe"
    CreateShortCut "$SMPROGRAMS\Pd-32bit\Pd-32bit.lnk" "$INSTDIR\bin\pd.exe"
  SectionEnd

  Section "${COMPONENT_DESKTOPSHORTCUT_TEXT}" DesktopShortcut
    CreateShortCut "$Desktop\Pd-32bit.lnk" "$INSTDIR\bin\pd.exe"
  SectionEnd
...
Section Uninstall
  ...
  Delete "$SMPROGRAMS\Pd-32bit\Uninstall.lnk"
  Delete "$SMPROGRAMS\Pd-32bit\Website.lnk"
  Delete "$SMPROGRAMS\Pd-32bit\Pd-32bit.lnk"
  RMDir "$SMPROGRAMS\Pd-32bit"
  Delete "$DESKTOP\Pd-32bit.lnk"
