...
!define PRODUCT_NAME "Pure Data 64-bit"
...
OutFile "/tmp/pd-${PRODUCT_VERSION}.windows-installer.exe"
InstallDir "$PROGRAMFILES64\Pd-64bit"
...
SectionGroup /e "${COMPONENT_GROUP_TEXT}"
  ...
  Section "${COMPONENT_STARTMENU_TEXT}" StartMenu
    ...
    CreateDirectory "$SMPROGRAMS\Pd-64bit"
    CreateShortCut "$SMPROGRAMS\Pd-64bit\Website.lnk" "$INSTDIR\${PRODUCT_NAME}.url"
    CreateShortCut "$SMPROGRAMS\Pd-64bit\Uninstall.lnk" "$INSTDIR\uninst.exe"
    CreateShortCut "$SMPROGRAMS\Pd-64bit\Pd-64bit.lnk" "$INSTDIR\bin\pd.exe"
  SectionEnd

  Section "${COMPONENT_DESKTOPSHORTCUT_TEXT}" DesktopShortcut
    CreateShortCut "$Desktop\Pd-64bit.lnk" "$INSTDIR\bin\pd.exe"
  SectionEnd
...
Section Uninstall
  ...
  Delete "$SMPROGRAMS\Pd-64bit\Uninstall.lnk"
  Delete "$SMPROGRAMS\Pd-64bit\Website.lnk"
  Delete "$SMPROGRAMS\Pd-64bit\Pd-64bit.lnk"
  RMDir "$SMPROGRAMS\Pd-64bit"
  Delete "$DESKTOP\Pd-64bit.lnk"
